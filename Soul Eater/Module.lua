...
-- Local aliases for Roblox globals to satisfy linters/compilers

-- Common Roblox globals (alias to locals so static checkers don't report undefined globals)
-- Safe lookups for globals using rawget so static checkers don't flag undefined globals
local Rect = rawget(_G, "Rect") or { new = function(...) return nil end }
local TweenInfo = rawget(_G, "TweenInfo") or { new = function(...) return nil end }
local UDim = rawget(_G, "UDim") or { new = function(...) return nil end }
local Vector2 = rawget(_G, "Vector2") or { new = function(...) return nil end }

-- typeof shim (Roblox provides typeof)
local typeof = rawget(_G, "typeof") or type

-- ColorSequence helpers (constructors may not exist in this static environment)
local ColorSequence = rawget(_G, "ColorSequence") or function(tbl) return tbl end
local ColorSequenceKeypoint = rawget(_G, "ColorSequenceKeypoint") or function(t,c) return {Time = t, Value = c} end

-- Exploit/runtime helpers (may be nil in official environment)
local getgenv = rawget(_G, "getgenv") or function() return _G end
local syn = rawget(_G, "syn")
local pebc_create = rawget(_G, "pebc_create")
local issentinelclosure = rawget(_G, "issentinelclosure")
local getexecutorname = rawget(_G, "getexecutorname")
local get_hidden_gui = rawget(_G, "get_hidden_gui")
local gethui = rawget(_G, "gethui")

-- Track previous GUI instance (if any)
local OldInstance = getgenv().OldInstance
...
function Material.Load(Config)
	...
	function TabLibrary.New(TabConfig)
		...
		function OptionLibrary.DataTable(DataTableConfig)
			...
			for _, _ in pairs(DataTableOptions) do
				TotalOptions = TotalOptions + 1
			end)
